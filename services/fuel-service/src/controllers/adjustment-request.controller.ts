...
import { createRequest, listRequests, approveRequest, rejectRequest } from '../services/adjustment-request.service'
...
export async function patchApprove(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.headers['x-user-id'] as string | undefined
    const userName = req.headers['x-user-name'] as string | undefined
    if (!userId || !userName) { res.status(401).json({ error: 'Thiếu thông tin xác thực người dùng.' }); return }

    const result = await approveRequest(req.params.id, userId, userName)
    res.json(result)
  } catch (err: unknown) {
    res.status((err as { status?: number }).status || 500).json({ error: (err as Error).message })
  }
...
export async function patchReject(req: Request, res: Response): Promise<void> {
  try {
    const userId = req.headers['x-user-id'] as string | undefined
    const userName = req.headers['x-user-name'] as string | undefined
    if (!userId || !userName) { res.status(401).json({ error: 'Thiếu thông tin xác thực người dùng.' }); return }
    ...
    const result = await rejectRequest(req.params.id, userId, userName, rejectionReason)
    res.json(result)
  } catch (err: unknown) {
    res.status((err as { status?: number }).status || 500).json({ error: (err as Error).message })
  }
